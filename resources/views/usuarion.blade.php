...
@section('seccion')
  <h3>Nuevo usuario.</h3>
  <form method="post" action="{{Route('usuario.insertar')}}" class="row g-3 needs-validation" novalidate>
    @csrf
    @foreach ($cempleado as $item)
      <div class="col-md-4">
        <label for="validationCustom01" class="form-label">Identificador</label>
        <input type="identificador" class="form-control" id="validationCustom01" name="identificador" value="{{$item->IdEmpleado}}" readonly>
      </div>
      <div class="col-md-4">
